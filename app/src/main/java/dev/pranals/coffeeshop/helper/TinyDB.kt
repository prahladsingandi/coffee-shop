...
class TinyDB(appContext: Context?) {
    ...
    fun getListObject(key: String?): ArrayList<ItemModel?> {
        ...
        val playerList: ArrayList<ItemModel?> = ArrayList<ItemModel?>()

        for (jObjString in objStrings) {
            val player: ItemModel? = gson.fromJson<ItemModel?>(jObjString, ItemModel::class.java)
    ...
    fun <T> getObject(key: String?, classOfT: Class<T?>): T? {
        ...
        val value: Any = com.google.gson.Gson().fromJson<T?>(json, classOfT)!!
